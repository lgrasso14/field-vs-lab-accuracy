...
negn <-  neg/tot
equaln <- equal/tot
countsn <- as.data.frame(rbind("pos" = posn,"neg" = negn, "equal" = equaln, tot))
...
db1s <- db1 %>%
  ...
  mutate(frequency = n()) %>%
  ungroup() %>%
  mutate("norm" = (normalized_frequency = frequency / total_count))

plot <- ggplot(db1s, aes(x= bins)) + 
  
  geom_bar(aes(y= after_stat(count))) + scale_y_continuous(limits=c(0,5000)) +
 ...
 # freq <- db1 %>% group_by(bins) %>% count((bins))   ## normalization 
 
 density_plot <- ggplot(db1s, aes(x = bins)) +
...
plot <-
  ...
  scale_y_continuous(aes(label=scales::percent(norm), vjust = -1)) + 
...
model <- lm(db1$labtcl2~db1$difference, data = db1)
print(summary(model))
